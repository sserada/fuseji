...
- **OpenAI Privacy Filter** / **GLiNER2-PII** などの汎用 LLM ベース redactor は英語中心で、マイナンバー / 法人番号 / 日本住所のような番号法対応エンティティに対する fail-closed 設計を持たない。また GPU 推論を前提とするためインライン経路の μs〜ms レイテンシ予算を超える
...
- `JP_ADDRESS` — 47 都道府県 + 市区町村 + 番地パターン（minimum viable regex）。既知制限: 北海道の条記法・漢数字・都道府県省略は未対応。dictionary-based 高精度版は v0.4 以降
- `CORPORATE_NUMBER`（法人番号 13 桁、国税庁公開仕様 checksum）
...
**v0.3（実装済み、次リリース予定）**: `CorporateNumberRecognizer`（13 桁法人番号 + checksum、opt-in、#126）・`JpAddressRecognizer`（47 都道府県 + 市区町村 + 番地、minimum viable regex、opt-in、#127）・`FakerStrategy`（Faker (ja_JP) で架空値置換、`[faker]` extra、#128）・OTel SDK 統合 example（mask_attribute helper、#129）・ingestion-callback の docker-compose 更新（v0.2 security middleware 利用例）。
