...
class Hash:
    ...
    cache: bool = False         # True でプロセス内 LRU を使い SHA256 を再利用 (#96)
```
...
- `cache=True` でプロセス内 LRU (max 8192) を共有し SHA256 計算を再利用可能。同一 surface が反復するログで CPU を削減する一方、cache キーとして PII surface がメモリに保持される。「detect, never retain」原則とのトレードオフを理解して opt-in (#96)
