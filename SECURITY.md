...
9. **`/detect` response default redact (#143)**: The FastAPI server's `POST /detect` returns `entities[].text` as `null` by default. The raw PII surface (email / phone / suspected My Number, etc.) is never inscribed onto the server-to-client network, log aggregators, APM, or browser devtools. Clients hold the original text themselves and can reconstruct from `type` / `start` / `end` / `score` / `recognizer` offsets alone. Raw surfaces can be opted in via `create_app(detect_include_surface=True)` or `FUSEJI_DETECT_INCLUDE_SURFACE=1` (intended only for trusted single-operator networks); even then `MY_NUMBER` / `CREDIT_CARD` / `CORPORATE_NUMBER` are returned as `<redacted>` constants. Addresses OWASP LLM02:2025 / CWE-200.
10. **DoS / resource consumption prevention**:
    - **`mask_json` recursion depth limit**: default 100 levels. Exceeding the limit fail-closes to `"[fuseji: too deep]"` without returning raw data, preventing stack overflow / infinite-recursion DoS (`Masker(max_json_depth=...)` is configurable).
